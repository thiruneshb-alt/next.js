...
function getIsPartialByte(isPartial: boolean) {
  return isPartial ? 0x7e : 0x23
}
...
function prependIsPartialByteToChunks(
  ...
  return [new Uint8Array([getIsPartialByte(isPartial)]), ...chunks]
}
...
function createPrerenderChunksAccumulator(): PrerenderChunksAccumulator {
  return {
    prerenderChunks: [],
    // In dev, we  alsocollect chunks that the render emits after aborting,
    // because they can contain debug info for promises that were hanging
